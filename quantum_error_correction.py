...
class QuantumErrorCorrection:
    ...
    def build_superoperator(self, tricky=True, sparse=False):
        logger.info(f"Building superoperator (tricky={tricky}, sparse={sparse})")

        if sparse:
            return self._build_superoperator_sparse(tricky)
        else:
            return self._build_superoperator_dense(tricky)

    def _build_superoperator_dense(self, tricky):
    ...
    def _build_superoperator_sparse(self, tricky):
        superoperator = sp.csr_matrix((self.dim ** 2, self.dim ** 2), dtype=np.complex64)
        kraus_sum = np.zeros((self.dim, self.dim), dtype=np.complex64)

        if tricky:
            no_error_syndrome = tuple([0] * len(self.stabilizers))
            logger.debug(f"Computing all-zero projector for syndrome: {no_error_syndrome}")
            all_zero_projector = self.compute_syndrome_projector(no_error_syndrome).cpu().numpy()

        for idx, (syndrome, recovery) in enumerate(self.recovery_map.items()):
            logger.debug(f"Processing syndrome {idx + 1}/{len(self.recovery_map)}: {syndrome}")
            projector_m = self.compute_syndrome_projector(syndrome).cpu().numpy()
            recovery_np = recovery.cpu().numpy()

            if tricky:
                combined_op = all_zero_projector @ recovery_np
            else:
                combined_op = recovery_np @ projector_m

            kraus_sum += combined_op.conj().T @ combined_op
            superoperator += sp.kron(np.conj(combined_op), combined_op, format='csr')

        logger.info(f"Sparse superoperator built: shape={superoperator.shape}, nnz={superoperator.nnz}")
        check_kraus_sum(self.dim, torch.from_numpy(kraus_sum))
        return superoperator